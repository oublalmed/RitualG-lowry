...
const mockSubscribers: Subscriber[] = [
  { id: '1', email: 'fatima@email.ma', source: 'Inscription', subscribedAt: '15 Jan 2025', isActive: true },
  { id: '2', email: 'houda@email.ma', source: 'Commande', subscribedAt: '02 Fév 2025', isActive: true },
  { id: '3', email: 'sanaa@email.ma', source: 'Pop-up', subscribedAt: '10 Mar 2025', isActive: true },
  { id: '4', email: 'rim@email.ma', source: 'Instagram', subscribedAt: '05 Mar 2025', isActive: false },
  { id: '5', email: 'nadia@email.ma', source: 'Inscription', subscribedAt: '20 Fév 2025', isActive: true },
  { id: '6', email: 'salma@email.ma', source: 'Parrainage', subscribedAt: '01 Mar 2025', isActive: true },
  { id: '7', email: 'yasmine@email.ma', source: 'Pop-up', subscribedAt: '12 Mar 2025', isActive: false },
  { id: '8', email: 'meryem@email.ma', source: 'Commande', subscribedAt: '25 Fév 2025', isActive: true },
];

const stats = [
  { icon: Users, label: 'Total abonnées', value: '1 248' },
  { icon: Mail, label: 'Actives', value: '1 185' },
  { icon: TrendingDown, label: 'Désinscriptions ce mois', value: '3' },
];
...
export default function NewsletterAdminPage() {
  ...
  const filtered = mockSubscribers.filter((s) => {
    const matchSearch = s.email.toLowerCase().includes(search.toLowerCase()) || s.source.toLowerCase().includes(search.toLowerCase());
    const matchStatus = statusFilter === 'all' || (statusFilter === 'active' ? s.isActive : !s.isActive);
    return matchSearch && matchStatus;
  });

  const handleExport = () => {
    const csv = ['Email,Source,Date inscription,Statut', ...filtered.map((s) =>
      `${s.email},${s.source},${s.subscribedAt},${s.isActive ? 'Active' : 'Désabonnée'}`
    )].join('\n');
            ...
            <tbody className="divide-y divide-[#C9A875]/10">
              {filtered.map((sub) => (
                <tr key={sub.id} className="hover:bg-[#FAF6EF]/50 transition-colors">
                  ...
                  <td className="px-4 py-3 font-inter text-xs text-[#3D2B1F]/50">{sub.subscribedAt}</td>
